...
describe("MelodicVector", () => {
    ...
    describe("is a Transformation", () => {
        ...
        it("can be applied to a melody that uses a string as a rest sybmol", () => {
            const melody = new Melody( helpers.getMelodicSteps([0, 7, "-", 5]), "-", MelodyType.MIDI );
            const transformedMelody = vector.applyTo(melody).steps.map(step => step.value);
